...
export default function MonthlyExpenseTrend({ data, 
    showLabels = true }: { data: JSONObject[], showLabels?: boolean }) {
    ...
    return (
        <div className={`grid ${showLabels ? "grid-cols-2 md:grid-cols-3 lg:grid-cols-3" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-2"} px-5 mt-3 gap-x-3 mb-4 items-center`}>
                ...
                <div className="w-full">
                    <ResponsiveContainer width="100%" height={400}>
                        ...
                        >
                            ...
                            {(categoryList ?? []).map(
                                (category: JSONObject, idx: number) => {
                                    return (
                                        <Bar
                                            key={`key_${idx}`}
            ...
            {showLabels && <div className="flex flex-col items-start justify-between w-64">
                <CategoryLegend />
            </div>}
        </div>
    );
