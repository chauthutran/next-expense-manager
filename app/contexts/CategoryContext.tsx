...
export const CategoryProvider = ({ children }: { children: ReactNode }) => {
    ...
    const fetchCategoryList = async () => {
        ...
        setError(null);
        try {
            const cachedData = await getData(Constant.INDEXED_DB_CATEGORY_LIST);
            if (cachedData) {
                setCategoryList(cachedData); // Use cached data if available
                ...
                if (!response.ok) {
                    ...
                    setProcessingStatus(Constant.FETCH_BUDGET_lIST_SUCCESS);
                    await saveData(Constant.INDEXED_DB_CATEGORY_LIST, categories); // Save fresh data to IndexedDB
                }
            }
        } catch (err) {
